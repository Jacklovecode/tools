...
def query_base(domain):
    print("原项目地址：https://github.com/wongzeon/ICP-Checker\n")
    while True:
        try:
            info = domain.replace(" ", "").replace("https://www.", "").replace("http://www.", "").replace("http://", "")
...
def data_saver(domain_list):
    ...
    try:
        ...
        print("** 备案信息登记表格已打开，无法写入文件。请关闭文件后重新执行！ **\n")
        return -1
    print(f"查询结果保存在：{file_path}\n")
    return 'OK'


def singel_main(target):
    cookie = get_cookies()
    domain = target
...
def parese_args():
    parse = argparse.ArgumentParser(epilog='\tExample: \r\npython '+sys.argv[0]+" -t baidu.com"
                                    , description="This is a tool about ICP by domain")
    parse.add_argument("-t", "--target", help=" single domain or single Full company name")
    parse.add_argument("-f", "--file", help="The file contains domain or Full company name")
...
if __name__ == '__main__':
    ...
    if args.file == None:
        if args.target!=None:
            singel_main(args.target)
    elif args.file != None:
        domains = getContent(args.file)
        file_main(domains)
    else:
        sys.exit(0)
